...
function CitizenPortal() {
  ...
  const fetchMyRecords = useCallback(async () => {
    ...
    try {
      ...
      const [complaintsResponse, emergenciesResponse] = await Promise.all([
        fetch(
          `http://localhost:5000/api/complaints/citizen/${encodeURIComponent(
            citizenId
          )}`
        ),
        fetch(
          `http://localhost:5000/api/emergency/citizen/${encodeURIComponent(
            citizenId
          )}`
        ),
      ]);
  ...
  const saveEmergency = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:5000/api/emergency/${editingEmergency.id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const submitComplaint = async (event: FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(
        "http://localhost:5000/api/complaints/citizen",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
