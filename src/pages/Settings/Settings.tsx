...
const Settings: React.FC = () => {
    ...
    const loadSettings = async () => {
      try {
        ...
        const [profileResponse, preferencesResponse, settingsResponse] =
          await Promise.all([
            fetch(`http://localhost:5000/api/admin/${savedAdminId}/profile`),
            fetch("http://localhost:5000/api/notification-preferences"),
            fetch(`http://localhost:5000/api/admin/${savedAdminId}/settings`),
          ]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const [profileResponse, preferencesResponse, settingsResponse] =
        await Promise.all([
          fetch(`http://localhost:5000/api/admin/${adminId}/profile`, {
            method: "PUT",
          ...
          fetch(`http://localhost:5000/api/admin/${adminId}/settings`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              language,
  ...
  const handleChangePassword = async (event: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:5000/api/admin/${adminId}/password`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
